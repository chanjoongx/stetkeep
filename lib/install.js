// stetkeep installer — cross-platform, stdlib-only
...
## stetkeep Integration (auto-appended)

This project uses the \`stetkeep\` protocol system.
...
export async function install({ pkgRoot, projectDir, mode = 'coexist', force = false, dryRun = false }) {
  ...
  if (fingerprints.claudeMd) {
    if (mode === 'coexist' || mode === 'merge') {
      ...
      if (!/(stetkeep|mdbrain|claude-brain|BRAIN\.md)/i.test(existing)) {
        ...
        log.skip(`skip: CLAUDE.md already references stetkeep`);
        skipped++;
      }
